...
@section('content')
    <div class="vironeer-steps-body">
        ...
        <form id="completeForm" action="{{ route('install.complete') }}" method="POST">
            @csrf
            <div class="mb-3">
                <label class="form-label">{{ admin_lang('Website name') }} : <span class="required">*</span></label>
                <div class="input-group">
                    <input type="text" name="website_name" value="{{ old('website_name') }}"
                        class="form-control form-control-md" placeholder="{{ admin_lang('Website name') }}"
                        autocomplete="off" required>
                    <span class="input-group-text"><i class="fas fa-globe"></i></span>
                </div>
            </div>
            <div class="mb-3">
                <label class="form-label">{{ admin_lang('Website URL') }} : <span class="required">*</span></label>
                <div class="input-group">
                    <input type="text" name="website_url" value="{{ old('website_url') ?? url('/') }}"
                        class="form-control form-control-md remove-spaces" placeholder="{{ admin_lang('Website URL') }}"
                        required>
                    <span class="input-group-text"><i class="fas fa-link"></i></span>
                </div>
            </div>
            <div class="mb-3">
                <label class="form-label">{{ admin_lang('Admin email') }} : <span class="required">*</span></label>
            ...
            <div class="mb-3">
                <label class="form-label">{{ admin_lang('Admin password') }} : <span class="required">*</span></label>
                <div class="input-group rtl">
            ...
            <div class="mb-4">
                <label class="form-label">{{ admin_lang('Confirm admin password') }} : <span
