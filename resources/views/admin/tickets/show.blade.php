...
@section('title', str(admin_lang('Ticket #{number}'))->replace('{number}', $ticket->id))
            ...
            <div class="row row-cols-1 g-3">
                                    ...
                                    <div class="mb-4">
                                        <div class="row row-cols-auto justify-content-between align-items-center g-3">
                                            <div class="col">
                                                <div class="tickets-user">
                                                    ...
                                                    @if (!$reply->user->isUser())
                                                        <div class="badge bg-primary ms-2">{{ admin_lang('Support') }}
                                                        </div>
                                                    @endif
                                                </div>
                                            </div>
                                            @if (!$loop->first)
                                                <div class="col">
                                                    <time
                                                        class="text-muted small">{{ dateFormat($reply->created_at) }}</time>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="ticket-content">
                                        ...
                                        <div class="tickets-paragrah">
                                            ...
                                            @if ($reply->attachments->count() > 0)
                                                <div class="mt-4">
                                                    <h6 class="text-dark mb-3">{{ admin_lang('Attached files') }}:</h6>
                                                    <div class="row g-2">
                ...
                <div class="col">
                    <div class="card custom-card">
                        <div class="card-body">
                            <div class="p-2">
                                <h5 class="mb-3">{{ admin_lang('Reply') }}</h5>
                                <form action="{{ route('admin.tickets.reply', $ticket->id) }}" method="POST"
                                            ...
                                            <div class="attachments">
                                                <div class="attachment-box-1">
                                                    ...
                                                    <div class="input-group">
                                                        ...
                                                        <button id="addAttachment" class="btn btn-secondary" type="button">
                                    ...
                                    <button class="btn btn-primary btn-lg">{{ admin_lang('Send') }}</button>
                                </form>
                            </div>
                        </div>
                    </div>
            ...
            <div class="card custom-card mb-3">
                <div class="card-body">
                    <div class="border-bottom pb-3 mb-3">
                        <h6 class="mb-0">{{ admin_lang('User details') }}</h6>
                    </div>
                    <div class="text-center">
                        <a href="{{ $ticket->user->getEditLink() }}">
                            <div class="mb-3">
                                <img src="{{ $ticket->user->getAvatar() }}" alt="{{ $ticket->user->getName() }}"
                                    class="rounded-circle" width="80px" height="80px">
                            </div>
                            <h5 class="text-dark">{{ $ticket->user->getName() }}</h5>
                    ...
                    <div class="p-2">
                        <div class="border-bottom pb-3 mb-3">
                            <div class="row row-cols-auto align-items-center justify-content-between g-2">
                                <div class="col">
                                    <h6 class="mb-0">
                                        {{ str(admin_lang('Ticket #{number}'))->replace('{number}', $ticket->id) }}
                                    </h6>
                                </div>
                                <div class="col">
                                    @if ($ticket->isOpened())
                        ...
                        <div class="border-top pt-3 mt-3">
                            @if ($ticket->isOpened())
                                <form action="{{ route('admin.tickets.close', $ticket->id) }}" method="POST">
                                    @csrf
                                    <button class="btn btn-danger btn-lg w-100 vironeer-form-confirm mb-3">
                                        <i class="fa-regular fa-circle-xmark me-1"></i>
                                        {{ admin_lang('Close ticket') }}
                                    </button>
                                </form>
                            @endif
                            <button class="btn btn-primary btn-lg w-100" data-bs-toggle="modal"
                                data-bs-target="#transferModal"><i
                                    class="fa-solid fa-share me-2"></i>{{ admin_lang('Transfer') }}</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="transferModalLabel">
                        {{ str(admin_lang('Transfer ticket #{number}'))->replace('{number}', $ticket->id) }}
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form id="transferForm" action="{{ route('admin.tickets.transfer', $ticket->id) }}" method="POST">
                    @csrf
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary btn-lg"
                            data-bs-dismiss="modal">{{ admin_lang('Cancel') }}</button>
                        <button class="btn btn-primary vironeer-form-confirm btn-lg">{{ admin_lang('Transfer') }}</button>
                    </div>
                </form>
            </div>
        </div>
