    ...
    <div class="cookies">
        <div class="cookies-img">
            <svg enable-background="new 0 0 80 80" height="80" viewBox="0 0 512 512" width="80"
                fill="{{ $settings->colors->primary_color }}" xmlns="http://www.w3.org/2000/svg">
                <g>
                    <path
        ...
        <div class="d-flex justify-content-center">
            <button id="acceptCookie" class="btn btn-primary hvr-radial-out px-5">{{ lang('Got it') }}</button>
            @if (settings('general')->terms_link)
                <a class="btn btn-outline-secondary btn-md px-5 ms-3"
                    href="{{ settings('general')->terms_link }}">{{ lang('More') }}</a>
            @endif
        </div>
    </div>
@endif
